...
class ExportGoodsController extends Controller
{
    ...
    {
        //var_dump(MarkUpGoods::find()->where(['is not', 'manufacturer_id', NULL])->exists());die();

        if (MarkUpGoods::find()->where(['is not', 'groups_id', NULL])->exists()){

            $goodsGroup = MarkUpGoods::find()->where(['is not', 'groups_id', NULL])->select('groups_id');
                ...
                if (ProductsImkuh::find()->where(['parser_status' => 1])->andWhere(['name' => $good['name_goods']])->exists()){
                    ...
                    if (file_exists($path)) {
                        ...
                        foreach ($files as $file) {
                            ...
                            file_put_contents('/var/www/imkuhru/data/www/imkuh.ru/images/products/' . $nameFile, file_get_contents($path . $file));
        ...
        if (MarkUpGoods::find()->where(['is not', 'manufacturer_id', NULL])->exists()){

            $goodsGroup = MarkUpGoods::find()->where(['is not', 'manufacturer_id', NULL])->select('groups_id');
